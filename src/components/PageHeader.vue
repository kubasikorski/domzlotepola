<template>
    <div class="relative z-20 md:container">
        <header class="fixed inset-x-0 top-0 justify-end items-center z-10 lg:flex lg:justify-between lg:static lg:py-6">
            <router-link :to="{name: 'home', params: {lang: lang}}" class="logo" v-on:click.native="hideMobileNav">
                <img src="../assets/images/logo-desktop.png" alt="Dóm Złote Pola">
            </router-link>

            <nav :class="{'is-open': mobileNav}">
                <router-link :to="{name: 'home', params: {lang: lang}}" v-on:click.native="hideMobileNav" class="hidden lg:inline-block">
                ...
                <router-link :to="{name: 'contact', params: {lang: lang}}" v-on:click.native="hideMobileNav" class="nav-item">Kontakt</router-link>
            </nav>
